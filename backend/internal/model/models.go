...
type Device struct {
	...
	// Topology (README 2.3.4.3):
	//   OCPP16:          connectorNo = connector count (default 2), evseNo unused (0)
	//   OCPP201/OCPP21:  evseNo (default 2) × connectorNo (default 1)
	ConnectorNo       int        `db:"connector_no"       json:"connectorNo"`
	EvseNo            int        `db:"evse_no"            json:"evseNo"`
	OwnerID           *string    `db:"owner_id"           json:"ownerId,omitempty"`
	OwnerName         string     `db:"owner_name"         json:"ownerName,omitempty"`
	TenantID          string     `db:"tenant_id"          json:"tenantId"`
	Status            string     `db:"status"             json:"status"`
	BootReason        string     `db:"boot_reason"        json:"bootReason,omitempty"`
	...
	// ConnectorStatuses is populated on demand (per-connector / per-EVSE status, 2.3.1)
	ConnectorStatuses []*ConnectorStatus `db:"-" json:"connectorStatuses"`
}

// ConnectorStatus holds the latest reported status of one connector
// (v16: evseId=0) or one EVSE+connector pair (v201: evseId>=1).
type ConnectorStatus struct {
	DeviceID    string    `db:"device_id"    json:"deviceId"`
	EvseID      int       `db:"evse_id"      json:"evseId"`
...
// Transaction201 is the OCPP 2.0.1 transaction (table transaction_201).
// transactionId is generated by the device (string); a transaction is bound
// by chargePointId + evseId + connectorId + transactionId (README 4.3.5/5.3).
type Transaction201 struct {
	ID            int64      `db:"id"              json:"id"`
...
// TransactionEvent201 is one recorded TransactionEvent message (4.3.5).
type TransactionEvent201 struct {
	ID             int64     `db:"id"               json:"id"`
	TenantID       string    `db:"tenant_id"        json:"tenantId"`
	ChargePointID  string    `db:"charge_point_id"  json:"chargePointId"`
	TransactionID  string    `db:"transaction_id"   json:"transactionId"`
	...
	TriggerReason  string    `db:"trigger_reason"   json:"triggerReason"`
...
type IDTag struct {
	...
	// Type is IdTokenEnumType for OCPP 2.x (README 2.3.4.4):
